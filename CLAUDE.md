...
9. **Mutation Mandate**: Load `mutation-tdd` after a test goes green. Delete the guard the test claims to cover, run only that test (`bunx jest <path> -t "<name>"`), confirm it FAILS, then `git checkout -- <file>`. A test that passes with its guard deleted proves nothing, and neither Jest nor the coverage number will tell you. Mandatory for async/lock tests, defensive branches, error and timeout paths, bridge-boundary error taxonomy, and any test written to close a coverage gap. **The `test:mutation:staged` pre-commit job is NOT this check** — `scripts/dlinter-mutation-staged.mjs` filters staged files down to the single hardcoded path `src/features/sync/native-foreground-sync-ticker.helpers.ts` and exits 0 for everything else, because dlinter drives Stryker through Vitest while this suite is Jest + `jest-expo`. See `docs/mutation-testing.md` and `.claude/skills/mutation-tdd/SKILL.md`.
