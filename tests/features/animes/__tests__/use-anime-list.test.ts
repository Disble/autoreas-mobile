...
jest.mock('../../../../src/infrastructure/db/native-runtime', () => ({
  ...
  useOptionalLiveQuery: (...args: unknown[]) => mockUseOptionalLiveQuery(...args),
}));
...
describe('useAnimeList', () => {
  ...
  it('filtra pseudo-días de estrenos usando dias[].dia', () => {
    ...
    expect(result.current.data.map((anime) => anime._id)).toEqual(['anime-today']);
    expect(result.current.data[0]?.dias).toEqual([{ dia: 'Ver hoy', orden: 1 }]);
  });
});
