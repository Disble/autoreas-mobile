...
function loadDefinedTask() {
  ...
  return (taskManagerModule.defineTask as jest.Mock).mock.calls[0][1] as () => Promise<string>;
}
...
describe('background sync task', () => {
  beforeEach(() => {
    ...
    jest.resetModules();
    (getTaskManagerModule().isTaskRegisteredAsync as jest.Mock).mockResolvedValue(false);
  });

  it('returns success when the background sync cycle records a handled failure', async () => {
    (getBackgroundSyncModule().runBackgroundSyncCycle as jest.Mock).mockResolvedValue({
      kind: 'failed',
      syncedCount: 0,
    });
    ...
    await expect(task()).resolves.toBe(getBackgroundTaskModule().BackgroundTaskResult.Success);
  });

  it('returns failed when the background task crashes unexpectedly', async () => {
    (getBackgroundSyncModule().runBackgroundSyncCycle as jest.Mock).mockRejectedValue(
      new Error('Unexpected crash'),
    );

    const task = loadDefinedTask();

    await expect(task()).resolves.toBe(getBackgroundTaskModule().BackgroundTaskResult.Failed);
  });
});