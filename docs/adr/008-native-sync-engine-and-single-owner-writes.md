...
1. **The background sync engine is native (Kotlin).** It owns transport, claim, staging, cursor, prune and the journal. No JS timer remains on the background sync path, so the paused-timer failure class leaves with it, together with `expo-background-task` and the JS-only bounds. The port is bounded and that is what makes the decision viable: in `staged` mode the background cycle applies no `bridge_changes` to `animes` — only the foreground drain hook does — so the domain apply logic stays in TypeScript, where the foreground already owns it.

   **Correction 2026-09-20.** An earlier sentence here claimed that "in `staged` mode the background cycle never writes `animes`". That sentence was taken from a code comment, not from the code, and the code says otherwise (`(source)`): `applyReconcileResponseWrites` runs the token writes **unconditionally, in both modes** — `await persistConfirmedAnimeTokens(writeDb, confirmedAnimeTokens)` and, per conflict outcome, `applyAnimeBridgeToken(...)` (`src/features/sync/reconcile.helpers.ts:238,240-241`) — and both issue `UPDATE animes SET bridge_modified_at = ?` (`src/infrastructure/db/anime-repository/anime-repository.ts:89-91`). So the background path in `staged` mode DOES write `animes`. Stated precisely, three facts hold:

   - The background cycle writes `animes` in `staged` mode, which falsifies the earlier sentence.
   - That write touches **one token column only** (`bridge_modified_at`), deliberately disjoint from the domain columns — `applyAnimeBridgeToken`'s own contract — so it is not itself a lost-update hazard on domain data.
   - What the native engine must still do is **own those token writes under the cycle lease**: defer them, re-home them, or perform them itself. What the single-owner actor (decision 3) removes is the shared write door's liveness coupling — a parked write stranding everything behind it — not this column; the token write can safely keep issuing `UPDATE animes SET bridge_modified_at = ?` beside the domain writers.

   Until the single-owner actor lands, there are two writers on `autoreas.db` on the token path, and the engine's slice must account for them rather than discover them.
