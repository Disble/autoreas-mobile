...
Cuando la PC emite eventos de cambio (`anime:changed`), se pueden recibir ráfagas. Para evitar N+1 escrituras y bloqueos en la UI:
...
Los payloads del WS deben ser validados de manera segura (`safeParse`).
```typescript
const WSEventSchema = z.object({
  type: z.literal('anime:changed'),
  payload: AnimeSchema.extend({
    version: z.number() // Reloj lógico/secuencia de la PC
  })
});
```
...
1. Llega evento WS de `anime_id = 10` indicando `nrocapvisto = 4`.
...
3. El evento del WS es droppeado silenciosamente.
