...
**2026-09-21, 1.4.0 cut from this state, and the 24 h window closed as VOID.** The window opened on 11:14 was read early at 14:47 and could not be read: `sync_runtime_status` was 3 h 35 min stale (`last_attempt_at = last_success_at = 11:12:40`) across 14 native cycles (12 `closed`, 2 `failed`), because the native engine writes no status row at all — `grep sync_runtime_status modules/**/*.kt` is empty and both native branches return from `engine.runOnce(...)` before any status write. So `consecutive_unclosed_cycles` can only ever read 0 and the §9 counter criterion is VOID, not passing; the falsifiable half is the journal, which is terminal for every cycle. Release 1.4.0 ships the state recorded above and moves both measured migration gaps to the backlog as T13 and T14 rather than shipping their unverified first implementations. Full evidence in `docs/mobile-background-sync-investigation-log.md`, entry of 2026-09-21 (release 1.4.0).
...
### T13 — the native attempt projects itself into `sync_runtime_status` (backlog, deferred out of 1.4.0)

- Surface: `src/features/sync/background-sync.helpers.ts`, `src/features/sync/notifee-foreground-service-adapter/notifee-foreground-service-adapter.helpers.ts`, one new shared native-attempt status helper, and their tests.
- Requirement: the native attempt must project itself into `sync_runtime_status` the way the JS cycle does, so the Settings surface stops lying and `consecutive_unclosed_cycles` stops being a constant.
- Contract, frozen so the next writer follows one variant: mirror `headless-sync-cycle.helpers.ts` around the native call. Before `engine.runOnce(triggerSource)` write `recordSyncAttemptStarted(rawDb, triggerSource, attemptedAt, null)` and then `recordCycleActive(rawDb, true)`, both awaited, with `attemptedAt` taken before the call. After the call, exactly ONE terminal write chosen by `result.outcome`: `closed` -> `recordSyncAttemptSucceeded(..., result.syncedCount, result.cycleId)`; `failed` or `abandoned` -> `recordSyncAttemptFailed(..., message, detail)`; `not_applicable` or `unavailable` -> only `recordCycleActive(rawDb, false)`, because nothing was claimed and an unreported start must not be counted as a leak on the next attempt. `message` is `result.errorName ?? 'native engine attempt failed'`. In `detail`, include `stage` only when `result.stage` belongs to the closed `SYNC_CYCLE_STAGES` vocabulary and `errorName` only when it belongs to the closed `SyncCycleErrorName` vocabulary; anything that does not fit is omitted and never fabricated, which is the policy already documented on `SyncAttemptFailureDetail`. The mapping belongs in one shared helper used by both callers.
- Evidence required: a device read showing `last_attempt_at`, `last_success_at` and `last_failure_message` moving with native cycles, and the counter becoming capable of a non-zero reading.
- Stated limit, deliberately out of the slice: the native result map carries an error class rather than the journal's free-text reason, so the recorded failure message is that class and the full reason stays in the native journal. Widening the Kotlin result map is its own decision.
- Preserved first implementation: `../autoreas-mobile-wip-t13-t14.patch` (1033 lines, unverified, outside the repository).

### T14 — the presence gate covers both triggers (backlog, deferred out of 1.4.0)

- Surface: `src/features/sync/background-sync.helpers.ts`, the extracted presence probe and its tests; the FGS adapter must keep its behaviour through the shared helper.
- Requirement: no attempt enters state without a present bridge, whichever trigger fired. The probe the FGS runner already uses — read `bridge_config` through `getBridgeConfigSnapshot(rawDb)`, call `bridgeClient.getStatus(...)` with `timeoutMs: ATTEMPT_PROBE_DEADLINE_MS`, count any HTTP answer including 401 as present, never throw — moves to a shared helper and gates `runBackgroundSyncCycle` as well: when presence is absent, return `{ kind: 'no_op', syncedCount: 0 }` without calling `engine.runOnce` and without any status write.
- Evidence required: with the bridge absent, a `background_task` run with the foreground service and its tick alarm verifiably alive produces no journal cycle and costs the probe budget.
- Preserved first implementation: `../autoreas-mobile-wip-t13-t14.patch` (1033 lines, unverified, outside the repository).
...
| T13 | backlog — deferred out of 1.4.0 on 2026-09-21 | Measured on device: `sync_runtime_status` was 3 h 35 min stale (`last_attempt_at = last_success_at = 11:12:40`, `last_failure_message=NULL`) across 14 native cycles (12 `closed`, 2 `failed`). `grep sync_runtime_status modules/**/*.kt` is empty and both native branches return from `engine.runOnce(...)` before any status write, so the Settings surface renders a frozen row — a regression against `v1.3.0` — and `consecutive_unclosed_cycles` can only ever read 0. Deferred rather than shipped unverified when the maintainer cut 1.4.0 from the working state; T13's section above holds the frozen contract. |
| T14 | backlog — deferred out of 1.4.0 on 2026-09-21 | Measured: the T6 presence gate exists only in the FGS runner, so `runBackgroundSyncCycle` calls the engine directly and a `background_task` run with the bridge absent entered `checked`/`sent` and paid `10033ms` (`failed to connect … after 10000ms`) at 14:42:53, while the `TICK_ALARM` at 14:42:41 was correctly refused. T6's `< 2 s, writes nothing` acceptance therefore holds only for the trigger that has the gate. |
