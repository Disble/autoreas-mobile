 ...
 *
 * The `undefined` entry IS the legacy sync-cycle envelope, and it is FROZEN. The bridge identifies
 * a cycle report by the ABSENCE of a `kind` key -- a frozen backward-compatibility rule, because
 * deployed mobile builds send no `kind` -- and an absent key reads as `undefined`. JSON cannot
 * represent `undefined`, so no body that declares a real kind (not even an explicit `null`) can
 * match that entry: it identifies absence itself, not any value a payload could carry.
 *
 * `episode_action` is the second entry, and it was the whole change that let chapter observations
 * flow again: the recorder started enqueuing and the flush started posting them off this same list
 * the moment the bridge answered that kind (bridge v1.15.0). There is deliberately no feature flag,
 * preference or build-time constant to remember in addition to it.
 *
 ...
 */
export const SYNC_DIAGNOSTICS_ACCEPTED_KINDS = [undefined, 'episode_action'] as const;
