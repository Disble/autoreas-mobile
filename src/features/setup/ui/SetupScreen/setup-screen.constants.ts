/** Provides the shared setup deep link scheme value. */
export const SETUP_DEEP_LINK_SCHEME = 'autoreas-mobile';
/** Provides the shared setup deep link host value. */
export const SETUP_DEEP_LINK_HOST = 'pair';
/** Provides the shared setup deep link supported version value. */
export const SETUP_DEEP_LINK_SUPPORTED_VERSION = '1';
/** Provides the shared setup screen default port value. */
export const SETUP_SCREEN_DEFAULT_PORT = '9876';
/** Provides the shared setup qr scan button label value. */
export const SETUP_QR_SCAN_BUTTON_LABEL = 'Escanear QR del Bridge';
/** Provides the shared setup pair button label value. */
export const SETUP_PAIR_BUTTON_LABEL = 'Emparejar Bridge';
/** Provides the shared setup invalid payload label value. */
export const SETUP_INVALID_PAYLOAD_LABEL = 'QR o deep link inválido';
/** Provides the shared setup invalid payload description value. */
export const SETUP_INVALID_PAYLOAD_DESCRIPTION =
  'Usá un código autoreas-mobile://pair?v=1 con ip, port y token.';
/** Provides the shared setup deep link detected label value. */
export const SETUP_DEEP_LINK_DETECTED_LABEL = 'Deep link detectado';
/** Provides the shared setup qr detected label value. */
export const SETUP_QR_DETECTED_LABEL = 'QR detectado';
/** Provides the shared setup autofill description value. */
export const SETUP_AUTOFILL_DESCRIPTION = 'Campos completados automáticamente.';
