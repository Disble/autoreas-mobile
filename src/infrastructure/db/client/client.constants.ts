...
export const SYNC_RUNTIME_STATUS_COLUMN_DEFINITIONS: readonly MissingColumnDefinition[] = [
  ...
  // The two counters stored beside `discarded` by the same patch helper, write and call site
  // (migration `0014` on a fresh install, these repair twins on a device already in the field --
  // `reconcileMigrationLedger` pins the migrator's gate, so this list is that device's ONLY
  // route). Nullable with no default for the same reason: NULL means "never measured".
  {
