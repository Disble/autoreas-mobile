...
6. **Mandatory JSDoc — repo-wide, not just helpers**: Every exported function needs a JSDoc block explaining what it does and why, and every exported variable needs one too. Enforced by `dharness/require-jsdoc` and `dharness/require-variable-jsdoc`. *Corrected 2026-08-29*: this constraint previously named `jsdoc/require-jsdoc` and scoped itself to `*.helpers.ts`. Both were wrong — that rule ID is not in this config, and helpers are only 69 of 277 JSDoc findings. Measured distribution: `tests/` 128, `src/` 88, `scripts/` 50, `plugins/` 8. By file kind: `*.helpers.ts` 69, plain `.ts` 70, `use-*.ts` 46, `*.schema.ts` 16, `*.tsx` 15.
   - *Standing debt*: the `dharness/*` layer went live 2026-08-29 against a codebase that predates it — **305 errors across 89 of 389 files**, 292 of them `dharness/*` (139 `require-variable-jsdoc`, 139 `require-jsdoc`, 7 `role-file-shape`, 4 `pure-index-barrel`, 3 `max-file-lines`).
   - *You cannot scope your way out of it*: the pre-commit `lint` job passes `{staged_files}` to ESLint, which lints the **whole file, never your diff**. Touch one line in a file with four undocumented exports and the commit fails on all four. "I did not touch that export" describes a commit that does not happen. Budget for it when you plan the edit rather than discovering it at the gate.
   - *Per-file, never in bulk*: pay this down only on files you are already editing. A sweeping doc-comment commit across the 89 files was explicitly rejected on 2026-08-29 in favour of gradual payoff.
   - *Interaction with constraint 7*: the TDD mandate sends you into `tests/` first, which carries the largest share (128). Expect the test file you create or update to need JSDoc before it will commit.
...
11. **Bridge Boundary Rule**: ALL communication with `autoreas-bridge` (HTTP and WebSocket) MUST go through the `BridgeClient` adapter in `src/infrastructure/api`. Feature code is FORBIDDEN from calling `fetch()`, instantiating `new WebSocket(...)`, or hand-building `http(s)://`/`ws(s)://` URLs. Consume `bridgeClient` (`pairDevice` / `listAnimes` / `getAnime` / `reconcile` / `openWebSocket`) instead, so base-URL resolution, auth injection, error taxonomy, and diagnostics live in ONE seam. The adapter (`src/infrastructure/api/**`) is the only place allowed to own transport. *⚠️ Enforcement status*: **none today — this is a convention you must uphold yourself.** The `no-restricted-syntax` "Bridge Boundary" rules named here were deleted when `eslint.config.mjs` moved to `dlinter-ts-react`, and nothing replaced them; measured 2026-08-12, raw `fetch`/`new WebSocket` under `src/features/**` lints clean. dlinter's `infrastructure` edge matches import specifiers, and `fetch`/`WebSocket` are RN globals that are never imported. See `ARCHITECTURE.md` § LLM Enforcement Barriers.
